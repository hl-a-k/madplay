...
struct option const options[] = {
  ...
  { "audio-type",		required_argument,	0,		 'j' },
  { "volume-type",		required_argument,	0,		 'k' },
# if defined(USE_TTY)
...
static float s_iAudioVolume[11] =  {-175, -23.5, -21, -18.5, -16, -13.5, -11, -8.5, -6, -3.5, 0};

void 
pause_handler(int signum)
{
    if(signum == SIGUSR1) {
        usleep(200000); 
    } else if(signum == SIGUSR2) { // 从环境变量读取音量
        int dataLen = 5;
        char data[5] = {0};
        FILE* datafd = fopen("/tmp/asi19_volume", "rb");
        if(datafd != NULL) {
            if (fread(data, 1, dataLen, datafd) == dataLen)
            {
                int iVolumeLevel = data[player.volumeType];
                int u8Level = 0;
                u8Level = iVolumeLevel / 10;
                u8Level = (iVolumeLevel % 10 == 0) ? u8Level : u8Level + 1;
                player.output.attamp_db = s_iAudioVolume[u8Level];  
                set_gain(&player, 0, 0);
            }  
            fclose(datafd);
        }
