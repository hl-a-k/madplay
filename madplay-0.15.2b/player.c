...
/*
 * NAME:	pause_audio()
 * DESCRIPTION:	pause playing the audio device for a while
 */
static
int pause_audio(struct player *player, int time)
{
  int result = 0;

  if (player->output.command) {
    union audio_control control;
    audio_control_init(&control, AUDIO_COMMAND_PAUSE);
    result = player->output.command(&control);
  }
  mad_timer_t mt;
  mt.seconds = time;
  mt.fraction = 0;
  mad_timer_add(&player->stats.global_timer, mt);
  sleep(time);
...
{
  int count, i, j, result = 0, first = 1;
